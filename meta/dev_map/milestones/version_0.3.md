...
**Проблемы текущего баланса**
   ...
   * {-} после **безопасной зоны** всё-таки замедлить набор опыта:
      * {-} в **Деревню Брейдвиль** герои должны приходить 3-им уровнем, если делали квесты, либо брать его по дороге в деревню;
      * {-} в **лагерь Стражей на берегу** герои должны приходить, где-то 4 уровнем (а не 5, 6);
      * {-} в **склепе** или **подземельях** берется 5;
      * {-} в **предместьях Хэмхолла** берётся 6;
      * {-} максимальный уровень (10) к финальному заданию должен браться только при выполнении всех побочных квестов;
...
### Последовательность работ по реализации версии

Задачи раздела **Важное** в приоритете.

Чтобы доработать героев, их способности и новые предметы, монстров и нпс нужно доделать **Тестовую арену**.

Чтобы избавиться от текущих проблем с балансом на карте и разрешить часть проблем концепции (раздел Нейтрализация наказаний или `/gamedes/problems.md`) нужно доделать **механизм тестирования актов и событий** и задачи раздела **Организация разработки**.

Последовательность реализации:
   1. Задачи раздела **Важное** в приоритете;
   * Доделать **механизмы тестирования**;
   * Закрыть раздел **Организация разработки**;
      ...
      * Можно начать дорабатывать локации и исправлять триггеры спавнов (`/bugs_list.md`), атак и сражений монстров;
